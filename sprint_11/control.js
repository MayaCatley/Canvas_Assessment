        ...
        if(name === "undo"){
            ...
            if (this.objectSet.length > 0) {
                if(this.objectSet[this.objectSet.length - 1].hasOwnProperty("id")){
                    ...
                    this.removedSet.push(penSet)
                }
                else {
                    console.log('undo werkin')
            ...
            if (this.removedSet.length > 0) {
                if(Array.isArray(this.removedSet[this.removedSet.length - 1])) {
                    console.log('we made it')
                    let redoId = this.removedSet[this.removedSet.length - 1][0]["id"]
                    console.log(redoId)
                    let redoPenSet = this.removedSet.filter(obj => obj["id"] != redoId);
                    console.log(redoPenSet)
                    redoPenSet[0].forEach(pen => this.objectSet.push(pen))
                }
                else {
